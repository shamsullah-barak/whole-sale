...
export const getUnitByIdAsync = createAsyncThunk(
  "units/getUnitById",
  async (unitId, { rejectWithValue }) => {
    try {
    ...
    builder
      ...
      .addCase(deleteUnitAsync.fulfilled, (state, action) => {
        state.units = state.units.filter(
          (unit) => unit.id !== action.payload && unit._id !== action.payload
        );
        state.error = null;
      })
      ...
      .addCase(updateUnitAsync.fulfilled, (state, action) => {
        const idx = state.units.findIndex(
          (unit) =>
            unit.id === action.payload.id || unit._id === action.payload._id
        );
        if (idx !== -1) {
